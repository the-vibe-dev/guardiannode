...
See [When Windows says "Protected your PC"](when-windows-says-protected-your-pc.md). Normal for beta; click More info → Run anyway.

### "User Account Control" loops
The installer needs admin rights. If clicking Yes doesn't proceed:
1. Right-click `GuardianNodeChildSetup.exe` → **Run as administrator**.
...
1. Reboot the PC.
2. Run `GuardianNodeChildSetup.exe` again — the installer detects an existing install and offers **Repair** and **Uninstall** options.
...
- The contents of the log file at `C:\ProgramData\GuardianNode\logs\` (redact any personal information first)
