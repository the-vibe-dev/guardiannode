...
1. Download `GuardianNodeServerSetup.exe` from [Releases](https://github.com/the-vibe-dev/guardiannode/releases).
2. Run the installer.
...
1. Download `GuardianNodeChildSetup.exe` to the kid's PC.
...
Try opening a monitored app on the kid's PC. Within ~10 seconds, an event should appear in the **Risk Feed** with risk level "none" or "low" (because nothing risky has happened yet). That confirms the pipeline works.
