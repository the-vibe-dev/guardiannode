...
Use the supplied `CAPTIONS.srt`. Export MP4/H.264 video with AAC voice audio at
1920x1080. Do not stretch still images to imply a control worked; the source UI
must be functional in the take.

## 5. Final watch-through

- Duration is 2:35–2:50 and voiceover is intelligible.
- The recording says mock mode is used and live mode is separate.
...
python scripts/check_build_week_video.py \
  /path/to/GuardianNode-Guardian-Review-Build-Week.mp4
```
