...
GuardianNode uses a signed, `HttpOnly` browser cookie over HTTPS and requires password
...
policy changes, device enrollment and revocation, parent-confirmed device
commands, archive operations, and backup configuration. Every denied challenge
and successful reauthentication is written to the audit log with actor, action,
source address, and target path.
