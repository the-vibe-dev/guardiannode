...
tray during install, adds all-user Startup entries so they launch for every
...
```cmd
GuardianNodeChildSetup-0.1.0.exe /VERYSILENT /MODE=separated /SERVERURL=http://192.168.1.176:8787 /PAIRCODE=506755
```
...
```bash
curl -fsSL https://raw.githubusercontent.com/the-vibe-dev/guardiannode/main/installer/server-linux/install.sh | sudo GN_NO_OLLAMA=1 bash
```
