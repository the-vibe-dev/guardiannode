...
```
[Child PC] agent ──HTTPS/LAN──> [Local backend] <──cookie auth── [Parent browser]
 (screenshots + OCR text)            │
                                     ├─ encrypted evidence at rest (master key)
                                     └─ Ollama vision/text inference (LAN, no SaaS)
```

- **Agent → backend**: device-token authenticated. The agent captures screenshots
  of monitored apps; the backend OCRs and classifies them. Runs on the family LAN
  or a self-hosted host. No third-party cloud is in the path.
...
- **Sniff agent→backend traffic.** Mitigation: deploy over HTTPS/TLS; evidence is
  encrypted at rest regardless. mDNS advertising can be disabled.
...
- **Read evidence directly.** Mitigation: evidence blobs and redacted text are
  encrypted at rest with the master key; the database stores ciphertext. The
  master key is required to decrypt and is not stored in plaintext alongside data.
...
- This is the risk highlighted by the FTC/UCL research linked in the readiness
  ...
  telemetry), encrypted evidence, **no raw keylogging**, capture scoped to
  monitored apps by default, and explicit parent opt-in (with child-visible tray
  wording) for full-screen capture.

## 4. Privacy-preserving design decisions

- **No raw keylogging.** We capture on-screen text in context (via screenshot OCR),
  not every keystroke.
- **Default capture scope is the monitored app/active window**, not the whole
  screen. Full-screen/background capture is an explicit opt-in and is recorded on
  every event as `capture_scope` (`active_window` / `monitored_app` /
  `full_screen_opt_in`).
- **Server-side redaction** strips secrets (passwords, API keys, 2FA codes,
  card/SSN numbers) from OCR'd text before storage; the redaction summary is
  attached to each event. See [REDACTION.md](REDACTION.md).
