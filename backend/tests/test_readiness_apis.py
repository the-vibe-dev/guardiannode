...
def _client(monkeypatch, tmp_path):
    ...
    r = client.post(
        "/api/auth/setup",
        json={"display_name": "Parent", "password": "correct horse", "recovery_code": "one two three"},
    )
...
async def test_text_event_passes_profile_custom_watch_phrases(db_session, monkeypatch):
    ...
    db_session.add(
        ChildProfile(
            ...
            display_name="Kid",
            age_group="10_13",
            custom_watch_phrases=["Lincoln Elementary"],
        )
    )
    db_session.commit()

    async def fake_classify_text(**kwargs):
        assert kwargs["custom_phrases"] == ["Lincoln Elementary"]
        return {
            "risk_level": "high",
            "score": 75,
            "categories": ["custom_watch"],
            "summary": "Detected parent watch phrase.",
            "evidence": ["Lincoln Elementary"],
            "recommended_action": "alert_parent",
            "model": None,
            "rules_triggered": ["custom_watch:lincoln elementary"],
            "confidence": 0.95,
    ...
    result = await event_ingest.ingest_event(
        ...
        payload={
            "profile_id": "child-1",
            "source_type": "browser",
            "redacted_text": "I go to Lincoln Elementary",
