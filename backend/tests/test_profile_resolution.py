...
async def test_text_event_uses_device_assigned_profile_watch_phrase(db_session):
    _mk_profile(db_session, "prof1", phrases=["lincoln elementary"])
    ...
    result = await event_ingest.ingest_event(
        ...
        payload={
            "source_type": "ocr",
            "redacted_text": "meet me after lincoln elementary tomorrow",
        },
        device_id="dev1",
    )
    db_session.commit()
...
async def test_screenshot_path_uses_device_assigned_profile_watch_phrase(db_session, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda b: "meet me after lincoln elementary tomorrow",
    )
