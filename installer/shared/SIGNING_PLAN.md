...
## v0.1 alpha — Document unsigned installer risk
...
- Mention the project status (alpha, open source, no signing yet) in the unsigned-installer guidance.
...
Eligibility:
- Open-source repo with AGPL-3.0 licensing
- Active maintainers
...
Options to compare:
- **Microsoft Store distribution**: avoids the unsigned-download warning path. Requires a $19 individual dev account or $99 company account, plus MS Store cert/review process. Apps go through store review.
- **EV code-signing certificate**: $300–600/year. Requires identity verification. EV certificates can establish stronger initial SmartScreen reputation.
...
- Work around SmartScreen via tricks (kernel-mode injection, etc.).
