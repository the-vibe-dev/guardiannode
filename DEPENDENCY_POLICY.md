...
## Requires maintainer/legal review as a hard dependency
...
Copyleft or source-available dependencies may still be acceptable, especially in
an AGPL-licensed project, but they require explicit maintainer review before
becoming default `pip install`/`npm install` dependencies.
...
7. Update release notes if the dependency materially affects licensing,
   privacy, or redistribution obligations.
...
Hashes for these binaries are pinned in the build scripts or release workflow
that downloads them. Generated release checksums live in `SHA256SUMS`; they are
not present in a fresh source checkout.

## Periodic review

- **Quarterly**: maintainers review for dependencies that have stopped being maintained
- **On every PR**: maintainer review for dependency/license changes
