...
Note on `zeroconf` (LGPL-2.1): We use it as an unmodified Python package via PyPI. LGPL allows this usage in an Apache-2.0 project as long as the LGPL component is not statically linked or modified. Users can replace it with an alternative mDNS implementation if their compliance requirements demand it.
...
Note on `PyInstaller` (GPL-2.0): PyInstaller's bootloader exception explicitly allows distribution of the resulting executables under any license. GuardianNode-built `.exe` files are Apache-2.0.
...
Run `scripts/check_licenses.py` to verify that all installed dependencies match the policy in [`DEPENDENCY_POLICY.md`](DEPENDENCY_POLICY.md). The CI workflow runs this on every PR.
